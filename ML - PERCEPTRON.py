...
def plotdecisionsurface(I,Y,epochs,bias,n,lr):
    ...
    for epoch in range(epochs):
        ...
        for i in range(n):
            x=np.array(I.iloc[i])
            o=classify(perceptron(w,x,bias))
            t=Y.iloc[i]
            #w=w+lr*(t-o)*x
            #bias=bias+lr*(t-o)
    ...
    ax2.plot(XD,YD,'k')
    color= ['red' if l == -1 else 'green' for l in Y]
    ax2.scatter(I['X1'], I['X2'], color=color)
...
def plot_batch_com(I,Y,epochs,bias,n,lr):
    ...
    print('Batch')
    print('Execution time: '+str(tm))
    print('Num of weight updated: '+str(nw))
